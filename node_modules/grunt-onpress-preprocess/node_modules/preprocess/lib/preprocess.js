...
function preprocess(src,context,type) {
  ...
  for( var prop in delim ) {
  
	  rv = rv.replace(getRegex(prop,'exclude'),function(match,test,include){
		return testPasses(test,context) ? '' : include;
	  });

	  rv = rv.replace(getRegex(prop,'ifdef'),function(match,test,include){
		test = (test || '').trim();
		return typeof context[test] !== 'undefined' ? include : '';
	  });

	  rv = rv.replace(getRegex(prop,'ifndef'),function(match,test,include){
		test = (test || '').trim();
		return typeof context[test] === 'undefined' ? include : '';
	  });

	  rv = rv.replace(getRegex(prop,'if'),function(match,test,include){
		return testPasses(test,context) ? include : '';
	  });

	  rv = rv.replace(getRegex(prop,'echo'),function(match,variable) {
